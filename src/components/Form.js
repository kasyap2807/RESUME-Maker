...
const Form = ({ data, setData, preset, setColor }) => {
  ...
  useEffect(() => {
    const fetchDataFromApi = async () => {
      try {
        ...
        const response = await axios.get(`http://localhost:8080/api/v1/GetData/${id}`);
        // console.log(response.data.jsonData);
        let json = response.data.jsonData
        json = json.replace(/Code123/g,"{");
        json = json.replace(/Code321/g,"}");
        json = json.replace(/\*/g, '"');
  ...
  const HandleFiletoDb = async () => {
    ...
    json = json.replace(/{/g, 'Code123');
    json = json.replace(/}/g, 'Code321');
    ...
    const sendDataToServer = async (dataToSend) => {
      try {
        // Replace 'YOUR_API_ENDPOINT/saveData' with the actual endpoint for storing data
        await axios.post('http://localhost:8080/api/v1/SetData', dataToSend, {
          headers: {
    ...
    await sendDataToServer({ "userid": id, jsonData: json });
  ...
  return (
    ...
    <div className='my-2'>
        <button className='btn btn-link'><Link to={`/Home/${id}`}>🔙 Back To Home Page</Link></button>
      </div>
